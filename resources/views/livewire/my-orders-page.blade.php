              ...
              <tbody>
                @foreach ($orders as $order)
                  @php
                    ...
                    $payment_status = '';
                    if ($order->status == 'new'){
                        $status = '<span class="bg-blue-500 py-1 px-3 rounded text-white shadow">{{ $order->status }}</span>';
                    }
                    if ($order->status == 'processing'){
                        $status = '<span class="bg-yellow-500 py-1 px-3 rounded text-white shadow">{{ $order->status }}</span>';
                    }
                    if ($order->status == 'shipped'){
                        $status = '<span class="bg-blue-500 py-1 px-3 rounded text-white shadow">{{ $order->status }}</span>';
                    }
                    if ($order->status == 'delivered'){
                        $status = '<span class="bg-green-700 py-1 px-3 rounded text-white shadow">{{ $order->status }}</span>';
                    }
                    if ($order->status == 'cancelled'){
                        $status = '<span class="bg-red-700 py-1 px-3 rounded text-white shadow">{{ $order->status }}</span>';
                    }

                    if ($order->payment_status == 'pending'){
                        $payment_status = '<span class="bg-green-500 py-1 px-3 rounded text-white shadow">{{ $order->payment_status }}</span>';
                    }
                    if ($order->payment_status == 'paid'){
                        $payment_status = '<span class="bg-green-600 py-1 px-3 rounded text-white shadow">{{ $order->payment_status }}</span>';
                    }
                    if ($order->payment_status == 'paid'){
                        $payment_status = '<span class="bg-red-600 py-1 px-3 rounded text-white shadow">{{ $order->payment_status }}</span>';
                    }
                  @endphp
